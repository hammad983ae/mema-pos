...
-- Create 10 sample employee profiles (using correct column names)
INSERT INTO public.profiles (user_id, username, full_name, email, position_type, phone, business_id, pos_pin)
SELECT 
  gen_random_uuid() as user_id,
  username, full_name, email, position_type, phone,
  (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() LIMIT 1) as business_id,
  '1234' as pos_pin
FROM (VALUES
  ('john.manager', 'John Smith', 'john.manager@company.com', 'manager', '(555) 111-1111'),
  ('sarah.cashier', 'Sarah Johnson', 'sarah.cashier@company.com', 'cashier', '(555) 222-2222'),
  ('mike.opener', 'Mike Wilson', 'mike.opener@company.com', 'opener', '(555) 333-3333'),
  ('lisa.upseller', 'Lisa Brown', 'lisa.upseller@company.com', 'upseller', '(555) 444-4444'),
  ('tom.cashier', 'Tom Davis', 'tom.cashier@company.com', 'cashier', '(555) 555-5555'),
  ('amy.opener', 'Amy Garcia', 'amy.opener@company.com', 'opener', '(555) 666-6666'),
  ('carlos.upseller', 'Carlos Martinez', 'carlos.upseller@company.com', 'upseller', '(555) 777-7777'),
  ('maria.cashier', 'Maria Rodriguez', 'maria.cashier@company.com', 'cashier', '(555) 888-8888'),
  ('david.manager', 'David Lee', 'david.manager@company.com', 'manager', '(555) 999-9999'),
  ('susan.cashier', 'Susan Taylor', 'susan.cashier@company.com', 'cashier', '(555) 000-0000')
) as v(username, full_name, email, position_type, phone);
