...
INSERT INTO public.products (name, description, price, cost, sku, barcode, category, track_inventory, is_active, business_id) 
SELECT 
  name, description, price, cost, sku, barcode, category, track_inventory, is_active,
...
SELECT 
  ...
  s.id as store_id,
  CASE 
    WHEN p.category = 'Electronics' THEN 25
    WHEN p.category = 'Accessories' THEN 50
    ELSE 30
...
WHERE p.business_id = (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() LIMIT 1)
  AND s.business_id = (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() LIMIT 1);