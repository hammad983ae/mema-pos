...
export default function CheckoutCustomer() {
  ...
  useEffect(() => {
    // Get business ID from POS session
    const posSession = localStorage.getItem('pos_session');
    if (posSession) {
      const session = JSON.parse(posSession);
      if (session.store?.business_id) {
        setBusinessId(session.store.business_id);
      }
    }
  ...
  const hasShippingItems = cartData.items?.some((item: any) => item.shipping_required === true) || false;
  ...
  const canContinue = selectedCustomer === null ? !hasShippingItems : (!hasShippingItems || customerHasAddress);
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span>Subtotal: ${cartData.subtotal?.toFixed(2) || '0.00'}</span>
              </div>
              ...
              <div className="border-t pt-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold">${cartData.total?.toFixed(2) || '0.00'}</span>
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="space-y-4">
            <CustomerSelection
              ...
              onSelectCustomer={(customer) => {
                ...
                console.log('State updated, selectedCustomer is now:', customer);
              }}
            ...
            {hasShippingItems && !customerHasAddress && (
              <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4">
                ...
                <p className="text-sm text-destructive/80 mt-1">
                  Your cart contains items that require shipping. Please add a complete address to the selected customer.
                </p>
              </div>
            )}
