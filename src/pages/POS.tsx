...
const POS = () => {
  ...
  const updateCustomerDisplay = () => {
    const displayData = {
      ...
      tipAmount,
      discountAmount,
      total: getTotalPrice() + (getTotalPrice() * 0.16) + tipAmount - discountAmount,
  ...
  useEffect(() => {
    const checkPOSAccess = async () => {
      ...
      try {
        ...
        if (!membership || !['owner', 'manager', 'office'].includes(membership.role)) {
          navigate('/pos/login');
          return;
        }

        // Find an active store
        const activeStore = membership.businesses.stores.find(store => store.status === 'active');
        if (!activeStore) {
          navigate('/pos/login');
          return;
        }
  ...
  const addToCart = (product: Product, customPrice?: number) => {
    ...
    setCartItems(prev => {
      const existingItem = prev.find(item => item.id === product.id && item.price === finalPrice);
      let newItems;
      if (existingItem) {
  ...
  const navItems = [
    ...
    { id: "receipts", label: "RECEIPTS", icon: Receipt, color: "text-blue-600" },
    { id: "mydash", label: "MY DASH", icon: Monitor, color: "text-purple-600" },
    { id: "clockin", label: "CLOCK IN", icon: Calendar, color: "text-green-600" },
    { id: "customers", label: "CUSTOMERS", icon: Users, color: "text-orange-600" },
    { id: "inventory", label: "INVENTORY", icon: Package, color: "text-red-600" },
    { id: "settings", label: "SETTINGS", icon: Settings, color: "text-blue-600" },
    { id: "reports", label: "X-REPORT", icon: FileText, color: "text-gray-600" },
    { id: "cash", label: "CASH MANAGER", icon: DollarSign, color: "text-green-600" },
    { id: "closeday", label: "CLOSE DAY", icon: Calendar, color: "text-red-600" },
    { id: "employees", label: "EMPLOYEES", icon: Users, color: "text-gray-600" },
    { id: "display", label: "CUSTOMER DISPLAY", icon: ExternalLink, color: "text-blue-600" },
  ];

  const openCustomerDisplay = () => {
    const displayUrl = `/pos/display?store=${encodeURIComponent(posUser?.store?.name || 'MemaPOS Store')}`;
    window.open(displayUrl, '_blank', 'width=800,height=900,resizable=yes,scrollbars=yes');
  };
          ...
          <div className="flex-1 flex flex-col bg-muted/20">
            ...
            <div className="p-4 bg-card/50 backdrop-blur-sm border-b border-border">
              <div className="flex gap-3 items-center">
                ...
                <BarcodeScanner 
                  onProductFound={(product) => addToCart(product, product.price)}
      ...
      case "receipts":
        return (
          <div className="flex-1 p-6 overflow-auto">
            <ReceiptManagement userRole={posUser?.role} />
          </div>
        );
      case "cash":
        return (
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                ...
                <Card className="p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
                  ...
                  <p className="text-sm text-muted-foreground">Add items manually</p>
                </Card>
                <Card className="p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
                  <Wrench className="h-12 w-12 mx-auto text-orange-600 mb-4" />
                  <h3 className="font-semibold mb-2">RECEIVE TRANSFERS/GOODS</h3>
                  <p className="text-sm text-muted-foreground">Process incoming stock</p>
                </Card>
                <Card className="p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
                  <Package className="h-12 w-12 mx-auto text-red-600 mb-4" />
                  <h3 className="font-semibold mb-2">LOSSES/DAMAGE</h3>
                  <p className="text-sm text-muted-foreground">Report damaged items</p>
                </Card>
                <Card className="p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
                  <Package className="h-12 w-12 mx-auto text-orange-600 mb-4" />
                  <h3 className="font-semibold mb-2">SEND MERCHANDISE</h3>
                  <p className="text-sm text-muted-foreground">Transfer to other stores</p>
                </Card>
                <Card className="p-6 text-center hover:shadow-lg transition-shadow cursor-pointer">
          ...
          return (
            <EmployeeDashboard 
              ...
              onSignOut={() => {
                ...
                toast({
                  title: "Signed out",
                  description: "You have been signed out of your employee dashboard.",
                });
              }}
            />
          );
        ...
        return (
          <div className="flex-1 p-6 overflow-auto">
            <div className="max-w-2xl mx-auto">
              <div className="mb-6">
                <h2 className="text-2xl font-bold text-foreground mb-2">Clock In/Out</h2>
                <p className="text-muted-foreground">Simple daily attendance tracking</p>
              </div>
              <Card className="p-8">
        ...
        return (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">{navItems.find(item => item.id === activeView)?.label}</h3>
        ...
        <div className="flex items-center justify-between px-6 h-full">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <div className="text-lg font-semibold text-primary-foreground">MemaPOS</div>
            </div>
      ...
      <div className="flex h-[calc(100vh-3.5rem)]">
        ...
        <div className="w-80 lg:w-96 xl:w-[420px] 2xl:w-[480px] bg-white dark:bg-card border-r border-border/50 shadow-2xl overflow-y-auto">
          <div className="p-6 lg:p-8 xl:p-10 space-y-8 lg:space-y-10">
            {/* Main Actions */}
            <div>
              <h3 className="text-lg lg:text-xl font-bold text-foreground mb-6 lg:mb-8 tracking-wide">MAIN ACTIONS</h3>
              <div className="grid grid-cols-2 gap-3 lg:gap-4">
                {navItems.slice(0, 4).map((item) => (
                  ...
                  >
                    ...
                    <span className="text-xs lg:text-sm text-center leading-tight font-bold tracking-wide">{item.label}</span>
                    {item.id === 'display' && <ExternalLink className="absolute top-2 right-2 h-4 w-4 opacity-60" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Management Tools */}
            <div>
              <h3 className="text-lg lg:text-xl font-bold text-foreground mb-6 lg:mb-8 tracking-wide">MANAGEMENT</h3>
              <div className="grid grid-cols-2 gap-3 lg:gap-4">
                {navItems.slice(4, 8).map((item) => (
                  <button
                    key={item.id}
                    onClick={() => item.id === 'display' ? openCustomerDisplay() : setActiveView(item.id)}
                    className={`group relative flex flex-col items-center gap-2 lg:gap-3 p-4 lg:p-6 rounded-2xl font-semibold transition-all duration-300 min-h-[100px] lg:min-h-[120px] ${
                      activeView === item.id && item.id !== 'display'
                        ? "bg-primary text-primary-foreground shadow-xl shadow-primary/25 transform scale-105"
                        : "bg-muted/30 text-foreground hover:bg-muted/50 hover:shadow-lg hover:transform hover:scale-105"
            ...
            {/* Day Operations */}
            <div>
              <h3 className="text-lg lg:text-xl font-bold text-foreground mb-6 lg:mb-8 tracking-wide">DAY OPERATIONS</h3>
              <div className="space-y-3 lg:space-y-4">
                {navItems.slice(8).map((item) => (
                  ...
                  >
                     ...
                     <span className="flex-1 text-left text-sm lg:text-base font-bold tracking-wide">{item.label}</span>
                     {item.id === 'display' && <ExternalLink className="h-5 w-5 opacity-60 flex-shrink-0" />}
                   </button>
                 ))}
               </div>
             </div>
