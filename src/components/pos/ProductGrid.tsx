...
export const ProductGrid = ({ searchQuery, activeCategory, onAddToCart }: ProductGridProps) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const inventoryLookup = inventoryData?.reduce((acc, item) => {
      ...
      const transformedProducts: Product[] = productsData?.map((dbProduct: DatabaseProduct) => ({
        ...
        category: dbProduct.product_categories?.name?.toLowerCase().replace(/\s+/g, '') || 'other',
    ...
    <div className="p-8 bg-muted/20">
      {filteredProducts.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-2xl font-semibold mb-4 text-foreground">No Products Found</h3>
