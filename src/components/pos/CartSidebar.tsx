...
import { Minus, Plus, Trash2, X, CreditCard, Percent, User, RefreshCw, History, Truck } from "lucide-react";
...
export const CartSidebar = ({
  ...
  const handleDiscountRemoved = (discountId: string) => {
    setAppliedDiscounts(prev => prev.filter(ad => ad.discount.id !== discountId));
  };

  return (
    <div className={`flex flex-col h-full bg-background ${isMobile ? 'animate-slide-up' : ''}`}>
      {/* Header */}
                  ...
                  <div className="mt-3 pt-2 border-t border-border">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <Label htmlFor={`shipping-${item.id}`} className="text-sm">
                          Ship this item
                        </Label>
                      </div>
                      <Switch
                        ...
                        onCheckedChange={(checked) => onUpdateShipping(item.id, checked)}
                      />
                    </div>
                    ...
                    {item.shipping_required && selectedCustomer && !selectedCustomer.address_line_1 && (
                      <div className="flex items-center gap-1 mt-1 text-xs text-amber-600">
                        <User className="h-3 w-3" />
                ...
                {(() => {
                  const totalSpare = items.reduce((sum, item) => {
                    if (item.minimum_price && item.minimum_price > 0) {
                      return sum + ((item.price - item.minimum_price) * item.quantity);
                    }
                    return sum;
                  }, 0);
