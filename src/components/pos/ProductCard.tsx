...
export const ProductCard = memo(({ product, inventory, onAddToCart }: ProductCardProps) => {
  return (
    <Card 
      className={`group relative bg-white dark:bg-card border-0 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden cursor-pointer ${
        !product.inStock ? "opacity-60" : "hover:-translate-y-2 hover:shadow-primary/10"
      }`}
    ...
    >
      <CardContent className="p-0 relative">
        ...
        <div className="relative aspect-[4/3] bg-gradient-to-br from-muted/20 to-muted/40 overflow-hidden">
          ...
          {inventory[product.id] !== undefined && inventory[product.id] > 0 && (
            <div className="absolute top-4 left-4">
              <Badge variant="secondary" className="bg-white/90 dark:bg-black/90 backdrop-blur-sm text-foreground border-0 shadow-sm">
                {inventory[product.id]} left
              </Badge>
            </div>
          )}
        </div>
