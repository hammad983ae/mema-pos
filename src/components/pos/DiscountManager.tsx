...
export const DiscountManager = ({
  ...
  const [showManagerOverride, setShowManagerOverride] = useState<string | null>(null);
  const [overrideReason, setOverrideReason] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const { toast } = useToast();

  const isManager = userRole === 'manager' || userRole === 'business_owner';
  ...
  const calculateDiscountAmount = (discount: Discount, amount: number): number => {
  ...
  const applyLoyaltyDiscount = async (loyaltyDiscountId: string) => {
    const loyaltyDiscount = loyaltyDiscounts.find(ld => ld.id === loyaltyDiscountId);
  ...
  const totalDiscountAmount = appliedDiscounts.reduce((sum, ad) => sum + ad.discountAmount, 0);
  ...
  return (
    <div className="space-y-4">
      ...
      {appliedDiscounts.length > 0 && (
        <div className="space-y-2">
          ...
          {appliedDiscounts.map((appliedDiscount) => (
            <div key={appliedDiscount.discount.id} className="flex items-center justify-between p-2 bg-success/10 border border-success/20 rounded-lg">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <div className="text-sm font-medium">{appliedDiscount.discount.name}</div>
                  {appliedDiscount.managerOverride && (
          ...
          {loyaltyDiscounts.map((loyaltyDiscount) => (
            <Card key={loyaltyDiscount.id} className="p-3">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm font-medium">{loyaltyDiscount.discounts.name}</div>
      ...
      <AlertDialog open={!!showManagerOverride} onOpenChange={() => setShowManagerOverride(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Manager Override Required</AlertDialogTitle>
            <AlertDialogDescription>
              This discount requires manager approval. Please provide a reason for the override.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="py-4">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => showManagerOverride && applyManagerOverride(showManagerOverride)}
      ...
      {totalDiscountAmount > 0 && (
        <div className="pt-2 border-t">
          <div className="flex justify-between text-sm font-medium">
            ...
            <span className="text-success">-${totalDiscountAmount.toFixed(2)}</span>
          </div>
        </div>
      )}
    </div>
