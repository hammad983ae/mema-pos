...
export const ChannelMembersManager = ({
  ...
  const canManageMembers = userRole === 'business_owner' || userRole === 'manager' || isChannelAdmin;
  ...
  const fetchAvailableMembers = async () => {
    ...
    try {
      ...
      const businessProfilesMap = new Map(businessProfilesData?.map(p => [p.user_id, p]) || []);

      const businessMembersWithProfiles = (businessMembersData || []).map(member => ({
                  ...
                  <div key={member.id} className="flex items-center justify-between p-2 rounded hover:bg-muted/50">
                    ...
                    {canManageMembers && member.user_id !== user?.id && (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => removeMemberFromChannel(member.id, member.user_id)}
                        disabled={loading}
                  ...
                  <div className="space-y-2">
                    {filteredAvailableMembers.length === 0 ? (
                      ...
                      filteredAvailableMembers.map((member) => (
                        <div key={member.user_id} className="flex items-center justify-between p-2 rounded hover:bg-muted/50">
                          <div className="flex items-center gap-3">
                            <Avatar className="h-8 w-8">
                              <AvatarFallback>
                                {member.profiles?.full_name?.charAt(0) || 'U'}
