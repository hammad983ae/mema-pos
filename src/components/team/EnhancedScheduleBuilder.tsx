...
import { Calendar, Clock, Layers, Send } from "lucide-react";
...
export const EnhancedScheduleBuilder = ({}: EnhancedScheduleBuilderProps) => {
  const { user } = useAuth();
    ...
    try {
      ...
      if (profiles) {
        ...
        const members = profiles
          ...
          .filter(
            (profile) =>
              profile.position_type === "opener" ||
              profile.position_type === "upseller",
          )
          .map((profile) => ({
            id: profile.user_id,
  ...
  const onDragEnd = (result: any) => {
    ...
    if (sourceDroppableId === "openers") {
      ...
      const sourceSlot = scheduleSlots.find(
        (slot) => slot.id === sourceDroppableId,
      );
      if (sourceSlot) {
    ...
    if (destDroppableId.includes("-")) {
      ...
      const targetSlot = updatedSlots.find(
        (slot) => slot.id === destDroppableId,
      );

      if (targetSlot) {
        // Check if slot has template requirements
        if (targetSlot.template) {
          const currentOpeners = targetSlot.assignedMembers.filter(
            (m) => m.position_type === "opener",
          ).length;
          const currentUpsellers = targetSlot.assignedMembers.filter(
            (m) => m.position_type === "upseller",
          ).length;

          if (
            draggedMember.position_type === "opener" &&
            currentOpeners >= targetSlot.template.required_openers
          ) {
            toast({
          ...
          if (
            draggedMember.position_type === "upseller" &&
            currentUpsellers >= targetSlot.template.required_upsellers
          ) {
            toast({
              title: "Assignment Limit Reached",
        ...
        if (sourceDroppableId.includes("-")) {
          const sourceSlot = updatedSlots.find(
            (slot) => slot.id === sourceDroppableId,
          );
          if (sourceSlot) {
      ...
      const assignedUserIds = Array.from(
        new Set(
          scheduleSlots.flatMap((slot) =>
            slot.assignedMembers.map((member) => member.id),
          ),
        ),
      );
      ...
      if (assignedUserIds.length > 0 && scheduleData.notifications?.push) {
        ...
        if (notificationError) {
          ...
          toast({
            title: "Warning",
            description:
              "Schedule published but push notifications failed to send",
            variant: "destructive",
          });
        }
      }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Create permanent shift templates and build schedules with drag &
            drop
          </p>
        </div>
